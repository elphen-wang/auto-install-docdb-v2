...
os.environ.setdefault('DOCDB_ROOT','/var/lib/docdb')#/var/www/html, /var/lib/docdb
...
class Config(object):
    ...
    defaults = dict(
        ...
        web_rouser = env('web_rouser', 'docdbro'),
        web_ropass = env('web_ropass', pwgen()),
        ...
        giturl = env('giturl','https://gitee.com/hk_donquixote/docdb2022.git'),
...
class Install(object):
    ...
    def install_docdb_files(self):
        ...
        if self.cfg.web_rouser in existing:
            self.warn('web password for {web_rouser} already set in {auth_file}')
        else:
            self.shell('htpasswd -m -b {auth_file} {web_rouser} {web_ropass}')
    ...
    def configure_mysql(self):
        ...
        #os.system('mysql -uroot -hlocalhost -p'+root_pass+' < /tmp/mysql-init.sql')
        #os.system('mysql -u{db_admuser} -p'+db_admpass+' '+db_name} < {srcdir}/DocDB/sql/CreateDatabase.SQL')
        #os.system('mysql -u{db_admuser} -p{db_admpass} {db_name} < /tmp/mysql-secgrp.sql')
